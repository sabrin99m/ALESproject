...
L = 64;                                % Filter length
SNR_1 = 20;                              % dB, snr = sigma_d^2/sigma_v^2;
SNR_2 = 15;

% Generate input signal (AR1 process)
a = 0.9;           % AR(1) coefficient
sigma = 1;         % Standard deviation of white noise

% Generate white Gaussian noise
w = sigma * randn(N, 1);

% AR(1) system: x(t) = a * x(t-1) + w(t)
% Using filter: b (numerator), a (denominator)
b = 1;
a_filter = [1, -a];

% Filter the noise to generate AR(1) process
u= filter(b, a_filter, w);
...
SNR_linear1 = 10^(SNR_1 / 20);         % SNR linearization
SNR_linear2 = 10^(SNR_2 / 20);
var_d = var(d_noiseless);              % power of the desired noiseless signal
sigma_v1 = sqrt(var_d / SNR_linear1);  % power of the system noise
sigma_v2 = sqrt(var_d / SNR_linear2);  % power of the system noise between iterations 22500 and 33750

v1 = sigma_v1 * randn(N, 1);              % Rumore 1 per tutto il segnale
v2 = sigma_v2 * randn(33750 - 22500 + 1, 1);  % Rumore 2 solo per l'intervallo centrale

% Costruzione del rumore totale v
v = v1;                                   % Inizializza con rumore 1
v(22500:33750) = v2;                      % Sovrascrive con rumore 2 nel range specificato
sigma_v = std(v);
